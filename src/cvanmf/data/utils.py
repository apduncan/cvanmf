...
def synthetic_dense(m: int = 100,
    ...
    Dense data is generated by making a W matrix with k signatures, and
    multiplying this with a randomly filled H matrix. Optionally, a proportion
    of the H matrices can be randomly set to 0 using h_sparsity. The extent to
    which features are shared between signatures is defined via
    shared_features. Each signature is initially assigned an even proportion
    of the m features (remainder spread as evenly as possible between
    them), so there are no shared features. Then if |k| is the number of
    features assigned to a signature, each signature is assigned
    |k|*shared_features randomly selected from the remaining features. This
    means the overlapping structure is potentially quite different from that
    of :func:`synthetic_blocks`.

    :param m: Number of features.
    :param n: Number of samples.
    :param h_sparsity: Proportion of H matrix to randomly set to 0
    :param shared_features: Amount of shared features to add to a
    signature, as a proportion of it's base size.
    :param k: Number of signatures.
    ...
    used instead.
    :param keep_mats: Return the H and W matrices used to generate the data
    in the other_metadata.
    """
